...
task :build_static => :clean do
  set_url(File.join (Dir.getwd + "_site"))
  compass('compile')
  jekyll('build')
end
...
task :create_post, [:date, :title, :category, :content] do |t, args|
  ...
  while File.exists?(post_dir + filename) do
    filename = post_date[0..9] + "-" + 
               post_title.gsub(' ', '_') + "-" + i.to_s + $post_ext
    i += 1
  end
  ...
  if not File.exists?(post_dir + filename) then
      File.open(post_dir + filename, 'w') do |f|
        ...
        f.puts "layout: post"
        f.puts yaml_cat unless yaml_cat.nil?
        f.puts "date: #{post_date}"
        f.puts "---"
        f.puts args.content unless args.content.nil?
      end  
...
EXCLUSION_LIST = [/.*~/, /_.*/, "javascripts?", "js", /stylesheets?/, "css", "Rakefile", "Gemfile",
                  /s[ca]ss/, /.*\.css/, /.*.js/, "bower_components", "config.rb", "README.md", "Gemfile.lock"]
...
task :check_links do
  begin
    ...
    abort 'Install anemone gem: bundle install'
  end
end
...
def compass(command = 'compile')
  (sh 'bundle exec compass ' + command) if $compass
end
